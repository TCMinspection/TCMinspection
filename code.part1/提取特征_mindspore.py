...
def create_data_transforms1():
    ...
    transforms = [
        ...
        vision.RandomColorAdjust(brightness=(0.7, 1),contrast=(0.4, 1)),
...
def compress_features_with_autoencoder(features, autoencoder_model, ckpt_path='D:/TCMinspection/autoencoder.ckpt'):
    """使用MindSpore自编码器降维"""
    try:
        # 转换为MindSpore张量
        ...
        if filename.endswith('.jpg') or filename.endswith('.png'):
            ...
            for region_id in range(9):
                ...
                if region_image is None or region_image.size == 0:
                    ...
                    print(f"降维前特征 shape: {features1.shape}")
                # 存储特征 - 对应PyTorch版本
                encoder_list.append(features1)  # 降维前的CNN特征

    # 保存特征和标签 - 对应两个版本
    if encoder_list:
        encoder_array = np.array(encoder_list)      # CNN+手工特征 (对应PyTorch版本)
        label_array = np.array(label_list)

        np.save('D:/TCMinspection/feature_mindspore.npy', encoder_array)  # 保存融合特征
        np.save("D:/TCMinspection/label_mindspore.npy", label_array)

        print("特征提取完成！")
        print(f"提取的特征数量：{len(encoder_list)}")
    else:
        print("未能提取任何特征！")
